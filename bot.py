...
from core.find_closest import TFIDFVac, Word2VecVac
...
def send_welcome(message):
    print("Start from user: ", message.from_user.username)
    bot.reply_to(message, "Привет. Напиши о себе и я подберу тебе самые подходящие вакансии.")

@bot.message_handler(func=lambda message: True)
def echo_all(message):
    print("Request from user: ", message.from_user.username)
