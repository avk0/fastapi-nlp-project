...
def scrape(phrase_to_search, con):
    ...
    cur = con.cursor()
    try:
        cur.execute("DROP TABLE hh_descriptions") # reset table # вынести в отдельный модуль model/queries.py
    except Exception as e:
        pass
    cur.execute("CREATE TABLE hh_descriptions(hhid, title, description, skills, url)")  # вынести в отдельный модуль model/queries.py

    ses = requests.Session()
    #ses.headers = {'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36'}
    ses.headers = {
    'HH-User-Agent': f'MyApp91019841/1.0 ({os.getenv("AGENT_EMAIL")})',
    #'HH-User-Agent': 'petprojectapp123124/1.0',
